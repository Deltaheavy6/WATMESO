...
def load_state_dict_flexible(model, state_dict):
    ...
    for name, param in model_state_dict.items():
        if name not in state_dict:
            print(f"Initializing {name} with appropriate method")
            if isinstance(param, nn.Parameter):
                param = param.data
            if param.dim() > 1:  # 对于权重参数，使用Xavier初始化
...
def train_nn(
    ...
    if model_path:
        ...
        load_state_dict_flexible(current_model,torch.load(model_path))
...
def parse_args():
    ...
    ASVSPOOF_DATASET_PATH = "datasets/ASV"
